    ...
    public PostDto.LikeResponse toggleLike(Long postId) {
        ...
        Post post = postRepository.findById(postId)
                .orElseThrow(() -> {
                    ...
                    return new ResponseStatusException(HttpStatus.NOT_FOUND, "Post not found: " + postId);
                });

        Optional<PostLike> existingLike = postLikeRepository.findByUserIdAndPostId(currentUser.getId(), postId);

        if (existingLike.isPresent()) {
            log.debug("Removing like for user {} on post {}", currentUser.getId(), postId);
        ...
        try {
            log.debug("Adding like for user {} on post {}", currentUser.getId(), postId);
            postLikeRepository.saveAndFlush(PostLike.builder().user(currentUser).post(post).build());
            long likesCount = postLikeRepository.countByPostId(postId);
            post.setLikes(likesCount);
            postRepository.save(post);
            return new PostDto.LikeResponse(likesCount, true);
        } catch (DataIntegrityViolationException ex) {
            log.warn("Duplicate like attempt prevented for user {} and post {}", currentUser.getId(), postId);
